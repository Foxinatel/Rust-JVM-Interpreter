...
impl Resolver {
  ...
  pub fn resolve(&mut self, (name, cf): (String, ClassFile)) {
    ...
    for i in constant_pool.as_slice() {
      match i {
        CpInfo::Fieldref { tag:_, class_index, name_and_type_index:_ }
        | CpInfo::Methodref { tag:_, class_index, name_and_type_index:_ }
        | CpInfo::InterfaceMethodref { tag:_, class_index, name_and_type_index:_ } => {
          ...
          if name == "java/lang/Object" || self.resolved.contains_key(name) { continue };
